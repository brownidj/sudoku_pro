...
class AnimalImageCache {
  ...
  static Future<Map<String, Map<int, ui.Image>>> _loadAll() async {
    ...
    final ocean = await _loadOceanImages();
    return {
      'simple': simple,
      'cute': cute,
  ...
  _loadNotesAll() async {
    ...
    final ocean = <int, Map<int, ui.Image>>{};
    ...
    final oceanNotes = await _loadOceanImages();
    for (final size in sizes) {
      ...
      ocean[size] = Map<int, ui.Image>.from(oceanNotes);
    }
    _notesCache = {
      'simple': simple,
      'cute': cute,
  ...
  static Future<Map<int, ui.Image>> _loadOceanImages() async {
    final images = <int, ui.Image>{};
    for (var d = 1; d <= 9; d += 1) {
      final name = _oceanName(d);
      final data = await rootBundle.load('assets/images/marine/${d}_$name.png');
      final image = await _decodeImage(data.buffer.asUint8List());
      images[d] = image;
    }
    return images;
  ...
  static String _oceanName(int digit) {
    switch (digit) {
      case 1:
  ...
  static String displayNameForDigit(String contentMode, int digit) {
    ...
    if (contentMode == 'ocean') {
      return _oceanName(digit).replaceAll('_', ' ');
    }
